...
public class GeminiService implements TripPlanProvider {
    ...
    private void appendHeader(StringBuilder itinerary, TripRequestContext context) {
        ...
        if (context.hasSelectedTypes()) {
            itinerary.append(styleLabel(context)).append(": ")
                    .append(context.getSelectedTypes())
                    .append("\n")
                    .append("Concrete style rule: every selected style must be expressed through real named places, not generic categories.\n");
        }
    ...
    private String styleLabel(TripRequestContext context) {
        if (context.getLanguageCode().equals("ru")) {
            return "Vybrannye stili";
        }
        if (context.getLanguageCode().equals("en")) {
            return "Selected tourism styles";
        }
        return "Secilen seyahat stilleri";
    }
